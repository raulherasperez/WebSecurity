...
function ModulePageXSS() {
  ...
  useEffect(() => {
    ...
    // El id del módulo XSS debe coincidir con el backend (ajusta si es necesario)
    axios.get(`${API_URL}/api/modulos/2`)
      .then(res => {
        setModulo(res.data);
  ...
  const xssQuizQuestions = modulo?.preguntasQuizCodigo || [];
      ...
      <main className="ModuleContent">
        ...
        <section>
          ...
          <div style={{ marginTop: 8, color: '#555', fontSize: '0.98em' }}>
            <MDEditor.Markdown
              source={
                nivel === 'facil'
                  ? 'Sin protección, vulnerable a cualquier XSS.'
                  : nivel === 'medio'
                  ? 'Filtrado parcial, aún vulnerable a vectores conocidos.'
                  : nivel === 'dificil'
                  ? 'Escape básico, pero quedan vectores avanzados.'
        ...
        <section>
          ...
          <button
            ...
            style={{ marginTop: 12 }}
            onClick={() => {
              localStorage.setItem('nivelXSS', nivel);
