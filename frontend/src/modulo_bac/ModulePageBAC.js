...
function ModulePageBAC() {
  ...
  const bacQuizQuestions = modulo?.preguntasQuizCodigo || [];

  useEffect(() => {
    setLoadingModulo(true);
    // El id del módulo BAC debe coincidir con el backend (ajusta si es necesario)
    axios.get(`${API_URL}/api/modulos/4`)
      .then(res => {
        setModulo(res.data);
        ...
        <section>
          ...
          <div style={{ marginTop: 8, color: '#555', fontSize: '0.98em' }}>
            <MDEditor.Markdown
              source={
                nivel === 'facil'
                  ? 'Sin control de acceso en backend, puedes ver y modificar cualquier perfil.'
                  : nivel === 'medio'
                  ? 'Solo el frontend protege, pero el backend sigue vulnerable.'
                  : nivel === 'dificil'
                  ? 'Solo puedes modificar tu perfil, pero puedes ver los de otros.'
                  : 'Solo puedes ver y modificar tu propio perfil, el backend comprueba todo.'
              }
